...
export default function ({ mode, trigger, initialValues, onFinish }: {
  mode: "edit" | "create";
  trigger: any;
  initialValues?: any;
  ...
  useEffect(() => {
    ...
    const targetDes = config.find(item => item.eventName === initialValues.eventName)?.eventDescription;
    setDes(targetDes || '选择触发事件后讲展示详情')

  }, [initialValues, config])

  return <ModalForm trigger={trigger} onFinish={async (vals) => {
    console.log(vals)
    if (!check(vals)) {
    ...
    autoFocusFirstInput title={isEdit ? "修改流水线" : "创建流水线"}>
    <ProFormText name="name" label="名称" required />
    <ProFormText name="description" label="描述" required tooltip="给自己写的，防止忘了" />
    <ProFormSelect name="eventName" label="触发事件" tooltip="选择后可以看到事件的说明" required request={async () => {
    ...
    <Form.Item label="事件说明"><div>{des}</div></Form.Item>
    <ProFormSwitch name="enabled" label="状态" required />
    <ProFormSelect name="deps" label="依赖" tooltip="依赖是 nodejs 包名，可以输入多个，将通过 pnpm install <依赖名> 来依次安装" mode="tags" placeholder={"请输入依赖"} />
  </ModalForm>
}
