...
export default function ({ pipeline, trigger }: {
  pipeline: any;
  trigger: any;
}) {
  ...
  return <ModalForm title="调试代码（请先保存）" onFinish={async (vals) => {
    if (!vals.input) {
      runCode();
      return true;
    } else {
    ...
    trigger={trigger}>
    <ProFormTextArea label='输入（json 格式）' name="input" tooltip="请输入给脚本传入的参数，JSON 格式，会注入到脚本的 input 中。" />
  </ModalForm>
