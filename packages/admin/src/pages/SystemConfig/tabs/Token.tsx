...
const columns = [
  ...
  {
    dataIndex: 'token', title: '内容', render: (token) => {
      return <Typography.Text style={{ maxWidth: 250 }} ellipsis={true} copyable={true}>{token}</Typography.Text>
    }
  },
  {
    title: '操作',
  ...
  return (
    <>
      <Card
        ...
        extra={
          <Space>
            <ModalForm title="新建 API Token" trigger={<Button type="primary"> 新建</Button>} onFinish={async (vals) => {
                ...
                Modal.info({
                  ...
                  content: (
                    <div>
                      ...
                      <p>PS：暂时没必要通过 API 开发自己的前台，后面会出主题功能（完善的文档和开发指南，不限制技术栈），届时再开发会更好。</p>
                      <p>
