...
- 上传文件夹会上传文件夹内所有文件到当前目录，保留层级关系（含子目录）。
- 上传文件会上传所选文件到当前目录。
- 删除文件：在左侧选中文件后，打开右上角「操作 → 删除文件」。

Windows 下上传文件/文件夹曾会因路径分隔符（`\`）创建目录失败，已修复（[#338](https://github.com/Mereithhh/vanblog/issues/338)）。单文件页面仍只保存在数据库里，不受多文件上传影响。
