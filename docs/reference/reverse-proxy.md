...
官方镜像里，内置 Caddy 已经把流量转到容器内的 `127.0.0.1:3000`，对外只需要映射 HTTP/HTTPS 端口。若你在**同一台机器**上用外层 Caddy/Nginx 反代源码启动的 Nest（默认端口 `3000`），或自己把 `3000` 暴露到了宿主机，可以把 API 绑到回环地址，这样只有本机反代能连上，其它网卡上的直连会被内核丢掉（[#488](https://github.com/Mereithhh/vanblog/issues/488)）。
...
改完后重启 VanBlog。启动日志会出现 `监听地址: 127.0.0.1:3000`。反代上游仍写 `127.0.0.1:<端口>`，与下面的示例一致。

官方 all-in-one 镜像一般**不要**再映射 `3000`。若把容器的 `3000` 映射到宿主机，同时又在容器内设了 `127.0.0.1`，Docker 从网桥打进来的连接到不了回环口，映射端口会连不上——这是预期的。外层反代请走映射的 80/443。
