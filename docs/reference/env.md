...
| `VAN_BLOG_LISTEN_HOST` | 否 | Nest/API 监听地址。留空则与升级前一样监听所有网卡。本机 Caddy/Nginx 反代时设为 `127.0.0.1`，只接受来自本机反代的连接。也可在 `config.yaml` 写 `listen.host`。见 [反代](./reverse-proxy.md#本机反代时只监听回环地址)。 | 未设置（所有网卡） |
