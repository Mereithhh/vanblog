...
根目录 `Dockerfile` 的前台阶段是 `node:18-alpine`。构建时会安装 `vips-dev` / `libc6-compat` / `fftw-dev`，并设置 `SHARP_IGNORE_GLOBAL_LIBVIPS=1`，让 `sharp@0.32.6` 使用官方 musl prebuild（避免 Alpine musl 版本号 `1.2.4_git*` 导致安装失败）。请继续用仓库自带的 `pnpm-lock.yaml` 和 `--frozen-lockfile`，不要把前台构建改回未处理 sharp 的旧 `node:18` 阶段。
