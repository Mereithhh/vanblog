...
- 标签
  ...
  标签不需要提前创建。新建 / 导入 / 修改草稿时，可以一次粘贴多个标签（英文/中文逗号、分号或换行分隔）；空格不会拆开多词标签。详见 [文章管理 · 标签](./article.md)：
...
点击编辑按钮，即可跳转到编辑器编辑该草稿。编辑器需要带上数字草稿 ID（`/editor?type=draft&id=数字`）；缺少或非法的 ID 会明确报错，而不会把已有正文写成空的。
