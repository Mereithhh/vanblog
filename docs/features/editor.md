...
编辑器宽度小于 800px 时（手机或窄窗），ByteMD 会切到「编辑 / 预览」分页，不再并排预览。此时工具栏会在切换按钮旁补一组常用操作：标题、加粗 / 斜体、链接、图片上传、无序列表、代码、引用。电脑端仍是完整分栏工具栏；表情、Mermaid、自定义高亮块、外链转存等仍只出现在桌面端，避免把整排按钮塞进手机。工具栏可以横向滑动，页面不会被撑出横向滚动。
...
编辑器现在支持偏好设置（右上角「操作 → 偏好设置」）：

- **保存后行为**：保存后留在编辑页，或返回上一页。
- **本地缓存**：默认关闭。开启后会把正文缓存在浏览器里，本地比服务器更新时用本地内容恢复。
- **软换行**：默认关闭，保持标准 Markdown：单独按 Enter 仍是同一段，需要行末两个空格（或空行）才换行。开启后，按 Enter 或粘贴多行正文时会自动补两个空格，写成标准软换行；预览和前台仍按 CommonMark 渲染（不会改用「单个换行符即换行」的插件）。打开或保存已有文章时不会整篇改写历史正文。
