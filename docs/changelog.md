...
- 后台「自定义路径名」标明对应 Hugo 的 `/post/:slug`：标签改为「自定义路径名 / slug」，tooltip / 占位提示从 Hugo 迁移时填旧 slug 以保留 SEO。导入 Markdown 时读取 Front Matter 的 `slug` / `pathname`（以及单段或 `/post/<slug>` 的 `url`）。没有全局固定链接模板，仍是按篇文章设置。[#487](https://github.com/Mereithhh/vanblog/issues/487)
...
- 文章 / 迁移 / SEO / FAQ：说明如何用「自定义路径名 / slug」（`pathname`）对应 Hugo `post = "/post/:slug"`，默认是数字 id，以及导入时 Front Matter 的映射。没有站点级 permalink 引擎。[#487](https://github.com/Mereithhh/vanblog/issues/487)
