...
在「站点管理 / 自定义页面」打开多文件页面的文件管理后，「上传文件」或「上传文件夹」失败（Windows 上常见 `ENOENT: mkdir`）。旧版本用 `/` 去拆 `path.join` 得到的 Windows 路径，父目录变成空字符串。已修复（[#338](https://github.com/Mereithhh/vanblog/issues/338)）。请升级到包含该修复的版本。

上传后的单个文件可在左侧选中后，用「操作 → 删除文件」删除。
