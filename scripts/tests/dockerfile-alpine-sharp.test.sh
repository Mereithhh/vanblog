...
# Isolate the WEBSITE_DEPS / WEBSITE_BUILDER stages so ADMIN/SERVER/RUNNER
# lines cannot satisfy the assertions.
WEBSITE_STAGES="$(awk '
  BEGIN { keep=0 }
  $0 ~ /^FROM / { keep=0 }
  $0 ~ /^FROM .* AS WEBSITE_DEPS/ { keep=1 }
  $0 ~ /^FROM .* AS WEBSITE_BUILDER/ { keep=1 }
  keep { print }
' "${DOCKERFILE}")"

if [[ -z "${WEBSITE_STAGES}" ]]; then
  fail "Dockerfile defines WEBSITE_DEPS / WEBSITE_BUILDER stages"
else
  pass "Dockerfile defines WEBSITE_DEPS / WEBSITE_BUILDER stages"
fi
...
# ENV must be set before pnpm install in the deps stage.
